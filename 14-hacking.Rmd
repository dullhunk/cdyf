...
It's very easy to overlook mistakes in your own writing. That's true of any written communication such as a covering letter, personal statement, email or any message that you write. Mistakes are particularly common in CVs (or resumes) because you can spend *hours* carefully honing the words and the formatting but not see a fatal error at the top of page one. You may need hack your CV and those of your peers, which could involve [kludges](https://en.wikipedia.org/wiki/Kludge) like the one shown in figure \@ref(fig:kludge-fig).
...
```
(ref:captionkludge) Your CV is a bridge which enables you to cross from where you are now to where you'd like to be in the future. The CV's in this chapter are all broken in some way, can you fix them? You may need to use [kludges](https://en.wikipedia.org/wiki/Kludge) and other ingenious hacks like the one shown here on the [Million Dollar Bridge](https://en.wikipedia.org/wiki/Miles_Glacier_Bridge) in Alaska. Public domain image of kludgy repairs by [Jet Lowe](https://en.wikipedia.org/wiki/Jet_Lowe) on Wikimedia Commons [w.wiki/3Uvn](https://w.wiki/3Uvn)
...
A useful hacking technique is to take somebody elses stuff and fix or improve it. That works for writing as well as actual `code`. The [dogfooding](#dogfooding) technique, we've already talked about in section \@ref(dogfooding) is a useful hacking technique which you can use by:
...
1. **Even Better If?** (EBI) What could be fixed or improved
