...
It's often better to describe what YOU did before you describe what the software, hardware or project did. Your reader is likely to be more interested in the former. Let's say you've developed a piece of software called `WidgetWasher`: instead of saying:
...
* `Designed and implemented an API which was built using a RESTful approach while pair programming with one other collaborator for two days at a hackathon`
...
The latter has all the same information, but by reversing the order, more emphasis is given to what you did, rather than what the software did.
