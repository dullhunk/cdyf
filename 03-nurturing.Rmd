...
One of the main target audiences for this textbook is [Generation Z](https://en.wikipedia.org/wiki/Generation_Z), born mid nineties to early noughties. There's some evidence as Gen-Z-er you have a higher probability of, as *The Economist* puts it, being “*Stressed, depressed and exam obsessed*” see figure \@ref(fig:stressed-depressed-fig).

```{r stressed-depressed-fig, echo = FALSE, fig.show = "hold", fig.align = "center", out.width = "99%", fig.cap = "(ref:captionstresseddepressed)"}
knitr::include_graphics("images/gen-z.jpeg")
```
(ref:captionstresseddepressed) Are you stressed? Depressed? Exam obsessed? According to *The Economist* members of Generation Z you are more likely to affected by stress, depression and exam obsession. [@generationz] Portrait of climate change activist Alienor Rougeot campaigning for [Fridays4Future](https://en.wikipedia.org/wiki/School_Strike_for_Climate) by Dina Dong on Wikimedia Commons [w.wiki/4ueR](https://w.wiki/4ueR) adapted using the [Wikipedia app](https://apps.apple.com/gb/app/wikipedia/id324715238)

If that's you then you are not alone. Stress, depression and results obsession have been around a while. Many people struggle with them so let's look at them in a bit more detail.
...
This chapter is based on my personal experiences of clinical depression, [antidepressants](https://en.wikipedia.org/wiki/Antidepressant) and the experience of close friends and colleagues who have been affected by generalised anxiety disorder.
...
```{r paulnurse-fig,echo=FALSE, fig.cap = "test caption"}
library(tweetrmd)
