...
Your future is bright, your future needs coding. However, if you prefer _Your Future_ to be in `dark-mode`, with [light text on a dark background](https://en.wikipedia.org/wiki/Light-on-dark_color_scheme) rather than the `default` **dark** text on a white background, we recommend using [darkreader.org](https://darkreader.org/) shown in figure \@ref(fig:darkmode-fig)
