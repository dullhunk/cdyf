...
Picking a specific technology can sometimes help you narrow down the options, for example by using these keywords in your search. The emphasis you put on technical skills in your degree will depend on which kind of employer you are applying to. Do they accept applicants with any degree, prefer a [STEM](Science, technology, engineering, and mathematics) degree or are they looking specifically for Computer Scientists, see figure \@ref(fig:degreeometer-fig).


```{r degreeometer-fig, echo = FALSE, fig.align = "center", out.width = "100%", fig.cap = "(ref:captiondegreeometer)"}
knitr::include_graphics("images/degreeometer.png")
```

(ref:captiondegreeometer) What kind of degree is the employer you are applying to looking for? Degree O'Meter by [Visual Thinkery](https://visualthinkery.com/) is licensed under [CC BY-SA](https://creativecommons.org/licenses/by-sa/4.0/). [Remix by Yours Truly](https://remixer.visualthinkery.com/r/HlXp6oNvrEDb5G3JmzFb), make your own at [remixer.visualthinkery.com](https://remixer.visualthinkery.com/)
...
```
 ... and so on. So locations can help you define (and potentially reduce) your search space. For example, if you're looking in Manchester, you can get started by looking at:

 * [industry-club.cs.manchester.ac.uk/manchester.html](https://industry-club.cs.manchester.ac.uk/manchester.html)
 
Picking a location can also help you identify networking opportunities for hidden, un-advertised vacancies and making speculative applications, described in section \@ref(speculative). You could find out about employers by attending free [meetup.com](https://www.meetup.com/) or [eventbrite.com](https://www.eventbrite.com/) events in your chosen location. See section \@ref(jonetwork) of Jonathan's story in chapter \@ref(jonathan). Since its a big subject, there's more information on searching by location in chapter \@ref(moving).
...
### Your University {#careerconnect}

Your University will have its own jobs board advertising vacancies targeted at graduates and undergraduates of that institution. At the University of Manchester, that's Career Connect, see figure \@ref(fig:careerconnect-fig).

```{r careerconnect-fig, echo = FALSE, fig.align = "center", out.width = "100%", fig.cap = "(ref:captioncareerconnect)"}
knitr::include_graphics("images/careerconnect.jpg")
```
(ref:captioncareerconnect) Most Universities have a jobs portal for students, at the University of Manchester it's [careerconnect.manchester.ac.uk](https://careerconnect.manchester.ac.uk) (login required) which is provided by [targetjobs.co.uk](https://targetjobs.co.uk)

### Any University {#studentjobs}

The following resources are aimed at students and graduates of _any_ University looking for jobs in Europe and beyond:
...
* [higherin.com](http://www.higherin.com/) is a leading UK job resource for undergraduates seeking placements and internships, formerly called ratemyplacement.com
...
* Your University: University jobs boards are good places to look for opportunities that are specifically targeted at students of the University where you are studying, for example:
    ...
    +  for any degree discipline in Manchester see [careerconnect.manchester.ac.uk](https://careerconnect.manchester.ac.uk/user/jobs.html) (login required)
...
knitr::include_graphics("images/google-job-search-2025.png")
```
...
* [welcometothehungle.com](https://uk.welcometothejungle.com) for people with 0-10 years experience. From engineering to sales, discover jobs & internships at London's most innovative companies.
...
* **LinkedIn.com** is the largest professional social networking form with more than 1 billion members in over 200 countries around the world. [@thanksabillion] LinkedIn allows you to create a semi-public or public CV and portfolio for employers, see figure \@ref(fig:linkedin-fig) and table \@ref(tab:linkedintable). Find out more about the pros and cons of creating a LinkedIn profile in chapter \@ref(linking): _Linking Your Future._
...
```
(ref:captionhoops) Are you ready to [jump through hoops](https://en.wiktionary.org/wiki/jump_through_hoops) like a circus animal? Hoop jumping is a time consuming activity that many employers will expect you to perform on their behalf. Hoops sketch by [Visual Thinkery](https://visualthinkery.com/) is licensed under [CC-BY-ND](https://creativecommons.org/licenses/by-nd/4.0/) via WAO [@plausibleutopias]
...
* For more horror stories see [google.com/search?q=unpaid+internships](https://www.google.com/search?q=unpaid+internships) [@everystudent]
...
Welcome to what can feel like an employer **black hole** we first met in section \@ref(blackhole) and illustrated in figure \@ref(fig:blackhole-fig).
...
The good news is, although applying for jobs can sometimes *feel* like sending your application into a black hole, it's not. Job applications don't disappear into black holes, they actually go into a funnel as shown in figure \@ref(fig:funnel-fig).

```{r funnel-fig, echo = FALSE, fig.align = "center", out.width = "98%", fig.cap = "(ref:captionfunnel)"}
knitr::include_graphics("images/funnel.png")
```

(ref:captionfunnel) A typical application funnel: the number of `Applications Submitted` (blue) far exceeds the number of people who are invited to `Interviews` (green). The number of `Interviews` exceeds the number of job `Offers` (orange). The size, shape and numbers on this funnel will vary depending on the state of the economy, employer and sector. This data is taken from [the-trackr.com](https://the-trackr.com) mentioned in section \@ref(studentjobs). In 2025, 1 in 75 trackr applications led to a job offer. The top 10% of candidates claimed 35% of interviews and 44% of offers. 51% of applicants received no offers at all. The average candidate submitted 66 internship applications with an interview rates of 7.2%, the interview-to-offer rate was 18.7%. [@trackrdata] Comparable rates have been reported elsewhere for year long placements and graduate roles. [@ise-data]
