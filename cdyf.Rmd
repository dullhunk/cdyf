...
* Most of the guidebook is generated from the [RMarkdown](https://en.wikipedia.org/wiki/Markdown) (that's the `*.Rmd` files). Markdown files are the only ones you should need to edit because everything else is generated from them including the `*.html`, `*.tex`, `*.pdf`,`*.epub` and `*.docx`  files
...
* From the command line `git clone https://github.com/dullhunk/cdyf.git` the repository and submit pull requests from your own setup
...
* Where is it too long and waffly or too short?
...
* Are there too many musical references or annoying emoji? Please bear in mind I'm trying to strike a light hearted and playful tone to make it readable 😜
...
1. Raise an issue via the project's issue tracker, such as [github issues](https://guides.github.com/features/issues/) [@githubissues]
...
So you worked in a team? `Worked` is vague. What was your role and contribution? How long did the project last? How many people were on your team?  What was the result?
...
### Your extras {#misc}
If you've anything else you want to highlight besides education, experience, projects and awards you *may* have room for one more section. Try to come up with a better name than `Miscellaneous` (which sounds like a dumping ground) or you might decide to have a dedicated `skills` section but see \@ref(mycvsk).
...
* you need to develop a better job search strategy
* you need to broaden your search see chapter \@ref(broadening)

Before we talk about broadening, lets looks at some basic job search strategies you can use to get started.
...
Starting a new job is a bit like starting a new relationship. You've searched for and found a partner. You've been through the courtship of recruitment, this might have been quick or may have had many rounds of first and second dates. Now you're both committed to each other and starting a serious relationship. Simply put, there are three scenarios for you as a new employee. You'll survive, thrive or dive.
