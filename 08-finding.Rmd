...
* Google job search aggregates content from many ([but not all](https://www.bbc.co.uk/news/technology-44853472)) of the sources listed above. You can use google job search use to find internships, placements and graduate jobs anywhere in the world, as well as saving vacancies and setting up job alert notifications by email. If you haven't used it already try the searches below. Unlike other sites, Google job search works by indexing embedded [microdata](https://en.wikipedia.org/wiki/Microdata_(HTML)) based on [schema.org/JobPosting](https://schema.org/JobPosting). Keywords like `job`, `intern` and `placement` in a vanilla google search will trigger the job search product:
