...
Lets imagine you have a handy command line utility `find_job` on your computer which knows about every job availabe, regardless of where it is advertised. Such a facility doesn't really exist, but lets pretend that it does. If you were to run utility with no parameters:

````python
find_job
````

You will be completely overwhelmed with the opportunities. It can be bewildering knowing where to start your job search, because the possibilities are endless.
...
````python
find_job --role product_management
````
...
````python
find_job --location Manchester
````

or

````python
find_job --location London
````
...
````python
find_job --salary £35,000
````

or

````python
find_job --salary £18,000
````

Setting a minimum salary will include or exclude certain employers from your search. You need to know how much you are worth to do this realistically, see section \@ref(undersell) and figure \@ref(fig:salaries-fig)
...
salaries %>% ggplot(aes(x=year, y=salary)) + geom_line() + geom_point() + ggtitle("UK graduate starting salaries (£)") 

```


(ref:captionsalaries) Average graduate starting salaries at the UK’s “top” employers 2007 to 2022, according to [highfliers.co.uk](https://www.highfliers.co.uk) [@highfliers2022] Since 2007, graduate salaries have risen from £24.5k to £32k in 2022. Note that these are *graduate* salaries, not internship or placement salaries which tend to be lower. Also note that “top” employers is a subjective judgement, lots of good smaller employers don't feature in this data and may pay less than the large multinational organisations that dominate this set of employers. The averages also hide significant variations between different sectors such as investment banking and the public sector, see also section \@ref(undersell)
