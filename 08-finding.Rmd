...
Lets imagine there is a handy AI-powered function in python called `find_jobs()` which knows *everything* about:
...
````python
from future import jobs

# call the fictional function
# what could possibly go wrong?
find_jobs()
````

...you will be completely overwhelmed with the results. It can be bewildering knowing where to start your job search, because the possibilities are endless.
...
1. 💰 by salary, see section \@ref(money)
...
# Fancy managing products?
find_jobs(role=product_management)

# Engineering makes the world go round
find_jobs(role=software_engineering)

# Wannabe a Data Scientist?
find_jobs(role=data_science)
...
# I've got the brains
find_jobs(sector=BigTech)

# You've got the looks
find_jobs(sector=Entertainment)

# Let's make lots of money
find_jobs(sector=FinTech)
````
...
# Do you love using Amazon Web Services?
find_jobs(skill=AWS)

# Howabout some cutting edge AI?
find_jobs(skill=DeepLearning)

# Is your name Noelle Pointer?
# Or are you Neil Pointer?
find_jobs(skill=C)
# See https://www.cdyf.me/hacking#neil-pointer
````
...
Pick a location. London? Manchester? New York? Silly Valley? The Outer Hebrides? Depending on the location, this will determine the options that are available:
...
# Hit the North!
# Northern Powerhouse
find_jobs(location=Manchester)

# Hit the South!
# Southern Powerhouse
find_jobs(location=London)
````
 ... and so on. So locations can help you define (and reduce) your problem space. For example, if you're looking in Manchester, you can get started at [git.io/manc](https://git.io/manc). Picking a location can also help you identify networking opportunities for hidden, un-advertised vacancies and making speculative applications. You could find out about employers by attending [meetup.com](https://www.meetup.com/) events in your chosen location.

### By network {#weakties}
The people in your network can help you find work. [It's not what you know, its who you know](https://en.wiktionary.org/wiki/it%27s_not_what_you_know_but_who_you_know), or so the cliché goes. The bigger your network, the more opportunities you'll know about. So a really basic strategy for finding jobs using your network. Remember, `find_jobs` does just know what you know, it also knows who is in your professional `network` and the employers *they* know about, so we could ask it to find opportunities in your professional and social networks:

````python
from future import jobs

# jobs in your network
find_jobs(network=True)

# jobs outside your network
find_jobs(network=False)
````
...
<!--script src="https://gist.github.com/dullhunk/45de6737e7cff0997c131acf8a1152ed.js"></script-->

````python
from future import jobs

# are you aiming high?
find_jobs(minimum_salary=£36000)

# or are you aiming lower?
find_jobs(minimum_salary=£18000)

# how low can you go?
# is that really a "job"?
# or just shameless exploitation?
find_jobs(minimum_salary=£0)

# if the salary is so competitive ...  
# why can't they just tell you what it is?
find_jobs(minimum_salary=competitive)
````
...
# looking for large multinationals?
find_jobs(minimum_employees=10000)

# howabout something more manageable?
find_jobs(maximum_employees=100)

# or even smaller still?
find_jobs(type=startup)
````
...
### By values {#by-values}
Besides the necessary value of making a profit, which is what commercial employers need to do to survive, what are the values of the employers you are interested in? How do they match your own? Reasonably or not at all?  How much of your soul do you need to sell? All of it, or just some of it?  Its a difficult question to compute, see section \@ref(soul), but let's imagine we can compute it:

````python
from future import jobs

# a reasonable match
find_jobs(values_match=70%)

# or no match at all?
# happy to sell your soul?
find_jobs(values_match=0%)
````

An employers `values` are often reflected in their `culture`. What is the employers culture like? Would you enjoy working there and would you felt like you belonged and fit in? How can you find this out? If you're struggling with these questions, you may need to do some more soul searching, see chapter \@ref(knowing).

Our `find_jobs()` function is useful, if only as a thought experiment, but there's still a strategic question of *when* you use the function and *how often* so lets look at those:
...
find_jobs(available=true)

# jobs that open later in the academic year
find_jobs(available=false)
````

Armed with knowledge of *when* to apply you can start to target employers. Timing is important because it some cases it will determine the type of organisation you apply to.
...
Salaries in the UK for interns (and graduates) range from [minimum wage](https://www.gov.uk/national-minimum-wage) to £50k and over, with everything in between. So beware of under-selling yourself, know your value, see \@ref(money). Some employers see students as a form of cheap labour that can be exploited because you're not “qualified” until you graduate. I'd think twice before working for such an employer, computing skills are in demand and there are plenty of other employers who will treat you with more respect. 📉
