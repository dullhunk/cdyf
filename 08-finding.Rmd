...
```
(ref:captiongooglejobsearch) Keywords like `job`, `intern` and `engineer` in an ordinary (vanilla) google search will trigger Google's job search product, an enhanced search feature that aggregates listings from many different jobs boards. Here's an example from clicking through on the results of a search for `software engineer manchester` ([google.com/search?q=software+engineer+manchester](https://www.google.com/search?q=software+engineer+manchester)). You can refine results by `title`, `location`, `date posted`, `type`, `company type` and `employer` shown across the top of the screenshot. See the text below for more examples.
