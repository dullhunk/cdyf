...
We all have strengths and weaknesses, and for students of Computer Science, sometimes their communication skills that leave the most room for improvement. Other chapters in this guidebook discuss the fundamental communication skills of [speaking](https://www.cdyf.me/speaking), [listening](https://www.cdyf.me/hearing) and [reading](https://www.cdyf.me/reading). This chapter looks at *written* communication because:

* every *engineer* is also a writer, and better engineers tend to have better written communication skills
* every *scientist* is also a writer, and better scientists tend to have better written communication skills too
...
* **OR** an “artier” and (allegedly) more “creative” [humanities](https://en.wikipedia.org/wiki/Humanities) pathway [@notcreatives]
...
1. humanities graduates who are articulate, well read, vocal, persuasive and write clearly but sometimes lack technical skills. In extreme cases, they can be scientificially, technologically and mathematically illiterate
...
* email and instant messaging, Discord, Microsoft Teams, Zoom, Slack etc
* Posting on social media: Bluesky, LinkedIn, Facebook, WhatsApp, Instagram, X, blogs, [medium.com](http://medium.com/), [substack.com](https://substack.com/) and whatever platform is fashionable this week.
...
* hacker news [news.ycombinator.com](https://news.ycombinator.com/) and [reddit.com](https://www.reddit.com/) etc
...
Writing has multiple uses, if it were a `function` or a `method`, it would be an [overloaded one](https://en.wikipedia.org/wiki/Function_overloading) performing different tasks. We need to examine these different use cases, because they are easily overlooked and taken for granted, especially by engineers and scientists. We read, write and rewrite stuff everyday without stopping to give it much thought. If you don't like writing, you need to learn to love it because it's actually your friend. Not just any friend, but your [Best Friend Forever](https://en.wikipedia.org/wiki/Best_friends_forever) (BFF), see figure \@ref(fig:bff-fig).
...
(ref:captionbff) Eight reasons why writing is your new Best Friend Forever (BFF): Communicating, see section \@ref(communicating). `Recording`, what happened see section \@ref(recording). `Leadership`: persuading other people to do something, see section \@ref(leadership). `Slowing down`: when your brain works too fast, see section \@ref(slowing). `Problem solving`: when your brain can't solve a problem (yet), see section \@ref(solving). `Process not product`, reflection and metacoginition, see section \@ref(product). `Therapy`: when your brain isn't helping because it makes you unhappy, anxious or depressed, section \@ref(therapy). `Speeding up`: when your brain too slowly, see section \@ref(accelerating). B.F.F. by [Visual Thinkery](https://visualthinkery.com/) is licensed under [CC-BY-ND](https://creativecommons.org/licenses/by-nd/4.0/)

So let's expand on each of the reasons why writing is your new BFF, as illustrated in figure \@ref(fig:bff-fig):

### Writing as Communication {#communicating}
Writing is communication, see figure \@ref(fig:comms-fig). Well Duh! I'm stating the obvious, because you will sometimes hear software engineers talking disparagingly about communication. For example, you might hear them complaining about writing the (*sigh*) `documentation` or having to explain things to (*yikes*) customers or stakeholders. 
...
(ref:captioncomms) Your writing will communicate better if you invest more time in rewriting to make it clearer. [Tin can telephone](https://en.wikipedia.org/wiki/Tin_can_telephone) sketch by [Visual Thinkery](https://visualthinkery.com/) is licensed under [CC-BY-ND](https://creativecommons.org/licenses/by-nd/4.0/)
...
What were you doing on the March 6th 2013? You probably don't know and I'll bet you would probably struggle to find out. I don't know what I was doing on March 6th 2013 either because I have no details written records for that day. If I was an [obsessive–compulsive diarist](https://en.wikipedia.org/wiki/Obsessive%E2%80%93compulsive_disorder) (OCD), I'd have a written record of what happened to me on that day. But I'm not, so I don't. The point here is that, writing doesn't just communicate it also *records* what has happened, see figure \@ref(fig:start-record-fig).
...
Have you ever felt moved or influenced by a piece of writing? Have you ever read something that motivated or influenced you to do something? Of course you have! If, as Harry Truman supposed, *leaders are readers* then it is also the case that  *writers are leaders too*, provided they write convincingly. [@truman]
...
* Getting a job interview, by crafting a persuasive covering letter and compelling CV 
...
Your brain tends to work *much* more quickly than your fingers can type or your hands can write. Your brain will often work much faster than you can speak as well. Writing can slow your thinking process down to something more manageable, by focussing your thoughts on a given topic or problem, see figure \@ref(fig:slowing-fig).
...
By the time you've written that difficult problem down, your brain may have figured it out. For example, as you're writing that [stackoverflow.com](https://stackoverflow.com/) question, your brain will sometimes magically solve it. This is much more likely to happen if you articulate the problem in words, either written down, spoken to a person or a rubber duck like the one shown in figure \@ref(fig:deducktion-fig)

```{r deducktion-fig, echo = FALSE, fig.align = "center", out.width = "100%", fig.cap = "(ref:captiondeducktion)"}
knitr::include_graphics("images/rubber-duck-debugging.jpg")
```
(ref:captiondeducktion) In software engineering, [rubber duck debugging](https://en.wikipedia.org/wiki/Rubber_duck_debugging) is a method of debugging code by articulating a problem in spoken or written natural language. Some people call it [deducktive reasoning](https://en.wikipedia.org/wiki/Deductive_reasoning). [CC BY-SA](https://creativecommons.org/licenses/by-sa/3.0/deed.en) rubber duck picture by [Tom Morris](https://commons.wikimedia.org/wiki/User:Tom_Morris) on Wikimedia Commons [w.wiki/4QHW](https://w.wiki/4QHW) adpated using the [Wikipedia App](https://apps.apple.com/gb/app/wikipedia/id324715238) 🦆


### Writing as a Process, not Just a Product {#product}

When you are writing a CV, résumé or a covering letter, the *product* of your writing is the most important output. But sometimes, it is the *process* of writing and [reflecting](https://en.wikipedia.org/wiki/Reflective_practice) that matters more than the product (output), see figure \@ref(fig:process-fig).
...
If you're using a machine or AI like ChatGPT to help you write (see section \@ref(machines)), you'll be bypassing some of the valuable *process* of writing that Simon talks about. [@youtube-peyton] You'll also be limiting yourself to its level: a machine which produces plausible human sentences rather than actual thoughts or knowledge. [@perret] Yes, writing can sometimes be painful and slow, but its one of the best ways to learn. If you're looking to improve your academic writing (essays, theses, dissertations etc), you will find [www.phrasebank.manchester.ac.uk](https://www.phrasebank.manchester.ac.uk/) useful. [@phrasebank]
...
(ref:captiondogfooding) Reading your own writing (aloud) is like [eating your own dog food](https://en.wikipedia.org/wiki/Eating_your_own_dog_food). It's a simple and proven technique for improving your written communication in job applications such as covering letters, personal statements and the like. Dogfooding by [Visual Thinkery](https://visualthinkery.com/) is licensed under [CC-BY-ND](https://creativecommons.org/licenses/by-nd/4.0/)
...
* morning pages, a unedited stream of conciousness that can help you become more creative, see figure \@ref(fig:jodi-picoult-fig) [@cameron;@burkeman]
...
```
(ref:captionbronte) The author [Charlotte Brontë](https://en.wikipedia.org/wiki/Charlotte_Bront%C3%AB) wrote the classic gothic novel *[Jane Eyre](https://en.wikipedia.org/wiki/Jane_Eyre)*. [@janeeyre] She just couldn't help it: “*I’m just going to write because I cannot help it*”. Just write, for some starting points see see section \@ref(dailywrite). Most computer scientists don't write or read enough material in natural languages like English, especially when compared to their fellow students in the humanities. Writing and reading more will make you a better communicator. Quotation via [bronte.org.uk](https://www.bronte.org.uk/), public domain image of a chalked portrait of Charlotte Brontë by George Richmond via Wikimedia Commons [w.wiki/9idq](https://w.wiki/9idq) adapted using the [Wikipedia App](https://apps.apple.com/gb/app/wikipedia/id324715238) ✍️
