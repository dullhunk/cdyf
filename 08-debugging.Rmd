...
1. **EDUCATION**: Is your year of graduation, degree program, University and expected (or achieved) degree classification clear? Have you mentioned things you are studying now, not just courses you have finished? See section \@ref(education)
1. **STRUCTURE**: Is the structure sensible? Is it in reverse chronological order? Most important (usually recent) things first? Not too many sections or anything missing? See section \@ref(structure)
...
Don't just TELL your reader, SHOW them. The numbers (`six` people, `four` months etc) and other evidence are a key part of your story that you want to tell potential employers. 

What's your story, coding glory? [@whatsthestory]
...
knitr::include_graphics("images/alan-turing-linkedin.jpg")
```

(ref:captioncvheader) Keep the header of your CV simple like Alan's example here. Just your name, email, phone number and any relevant links are all you really need. If you choose to have a public digital profile such as  `linkedin.com/in/alanturing` here, your header is a good place to link to it, see section \@ref(portfolio). You might also include a timestamp like `last updated 1954-06-07` in the footer. Any additional information risks wasting valuable space and distracting your reader from much more important things you need to tell them. [Less is more](https://en.wikipedia.org/wiki/Less_(Unix)).

Your header doesn't need to include any more information than your name, email, phone and any links. This means your birth date, marital status, gender, photo^[different countries have different conventions when it comes to passport-style photos on a CV. In the UK what you look like should *not* be a factor in an employers decision to interview you, so leave it out] and home address aren't relevant. You don't need to give multiple phone numbers or emails either, just one of each will do. If an employer wants to invite you to an interview, they'll get in touch by email, phone (or possibly LinkedIn, github etc) so other contact details such as your postal address are an irrelevant distraction at this point. After your header I suggest you have around five sections in the body as follows:

### Your body {#body}
Technical recruiters at Google and TikTok (see figure \@ref(fig:lopez-vid)) suggest you have just five sections in the body of your CV. Headings use lots of space so should be used sparingly, especially on a one page résumé. Your five sections should cover some or all of the following:

1. 🎓 `EDUCATION`: the formal stuff, see section \@ref(education)
1. 💰 `EXPERIENCE`: paid work, see section \@ref(experience)
1. 💪 `PROJECTS`: personal, school, social, side or University projects see section \@ref(projects)
...
### Your education, education, education {#education}

Unless you have a *significant* amount of `EXPERIENCE`, as a student or graduate the `EDUCATION` section of your CV should be the first “real” section, after the header. Your education section is important, see figure \@ref(fig:blair-fig), so your description of it needs to strike a balance between:
...
* Was their conflict in the team? How did you resolve it? 
...
These titles all sound like dumping grounds for your leftovers, added as an afterthought. They deserve a more interesting title. If you're thinking of having a section dedicated to

* `SKILLS` see section \@ref(care)
* `PERSONAL STATEMENT` see section \@ref(bullshit)
* `HOBBIES AND INTERESTS` see figure \@ref(fig:quidditch-fig) 
* `LANGUAGES` might not deserve their own section, but if you are multilingual, you could potentially fit the natural languages you speak in a `SKILLS` section
...
* avoid vague metrics of skill like `beginner`, `intermediate` and `advanced` as these are very subjectivfe and won't help the reader much. You might under-estimate or over-estimate your skill level anyway. Some people choose to quantify coding skills with `years of experience` or the `lines of code` (LoC), but these metrics are flawed. [@linesofcode] 
...
Making your CV look good can take ages, but a well presented CV will stand out. It is worth making an effort to style carefully and consistently. The [typography](https://en.wikipedia.org/wiki/Typography) of your CV matters because it helps conserve your readers attention, a finite resouce that you don't want to waste. A poorly presented CV is the equivalent of turning up to an interview in flip-flops and shorts. [@whytypographymatters]
...
Lots of *seemingly* trivial typographical details like this all add up to make your CV look more professional. Neglect these details at your peril.
...
```
(ref:captionlovelace) Adding links is a good way to augment your CV. If you're adding LinkedIn, make sure you [customise your default public profile URL](https://www.linkedin.com/help/linkedin/topics/6042/6054/87), (the `.../in/handle`) to remove the randomly generated alphanumeric string at the end, like the `038b37` example here. [@customlinkedin] You can also remove any ugly `http`, colons `::`, forward slashes  `//`, `www` and trailing `/` in URLs which are distracting noise. Just make sure links are clickable in the pdf, don't [404](https://en.wikipedia.org/wiki/HTTP_404) if they are followed and work when printed on paper too. Neither do you need to waste valuable space telling people what the link is, like in the first example, the domain name already tells you that it is a LinkedIn profile.
...
* your hyperlinks are paper-proof: links with opaque titles like  “[My LinkedIn](linkedin.com/in/duncanhull)”, “[click here](https://www.w3.org/QA/Tips/noClickHere.html)”, and “[My Projects on Github](https://github.com/dullhunk)”  won't work well when printed on paper unless you explicitly spell the URL out like this: 
...
It's a good idea get feedback from as many different sources as you can on your CV. By *sources* I don't just mean humans, but also robots. Larger employers routinely use automated [Applicant Tracking Systems](https://en.wikipedia.org/wiki/Applicant_tracking_system) (ATS) to log and trace your application which sometimes struggle with two column formats (see below). These “résumé robots” (if you like) are unlikely to have arms and legs like the one in figure \@ref(fig:machines-fig), but they *will* be looking for keywords and standard headings in your CV. [@airesume] You can get automated feedback from a range of different software systems, though it is a good idea to remove any personal information like phone numbers and emails before using these free services. You might also want to check what their privacy policy says about what they do with your personal data. Résumé robots include:

* [careerset.com](https://careerset.com), a free service provided by a UK based company, careerset Ltd. If you're a University of Manchester student, you can login using your Uni credentials at [careerset.com/manchester](https://careerset.com/manchester)
...
## CV checklist: Self-debugging known errors {#checklist}

It can be difficult to be **both** an *author* AND an *editor* of the same document, and your CV is no exception. This simple checklist below will help you start to debug common known errors that you may have overlooked as an author. For the *unknown* errors, you'll need help from someone (or something) else who play the role of editor, see figure \@ref(fig:unknown-error-fig).
...
```
(ref:captionunknownerrors) This self-debugging checklist below will help you identify *known errors* in your CV. To debug the more challenging *unknown errors*, see section \@ref(cvswap) below. [CC BY-SA](https://creativecommons.org/licenses/by-sa/4.0/deed.en) screenshot of *something went wrong* by Revansx on Wikimedia Commons [w.wiki/9hZD](https://w.wiki/9hZD)
...
1. 🔲 Have you made it clear what you have actually done using **prominent** `verbs`? See chapter \@ref(verbalising)
...
1. 🔲 Does it have a good, clear structure? Not too many headings, around five sections for a one-pager see section \@ref(header)?
...
1. 🔲 Have you used past tense consistently throughout e.g. `developed` not `develop`, `collaborated` not `collaborate`
1. 🔲 Is the formatting consistent, no random changes of font half-way through? Dates always in the same place? 
1. 🔲 On your two page version, does the page break fall in a sensible place, not halfway through a section? 
1. 🔲 One your one page version, have you wasted any space by having one word taking up a whole line? Can you rephrase the sentence so it fits on one line rather than taking up two? Can you merge sections to save valuable space?
...
1. 🔲 Have you tailored your CV to the job description? While the basic facts won't change much, you could emphasise (or de-emphasise) some `PROJECTS` and `EXPERIENCE` depending on the roles you are applying for, rather than having a generic *one-size-fits-all* CV. 

For the next debug step, you'll need some help from a third party.

## CV checklist: Peer-debugging unknown errors {#cvswap}

Now that you've [self-assessed](https://en.wikipedia.org/wiki/Self-assessment) your CV, swap it with somebody you trust and check each others work. This [peer assessment](https://en.wikipedia.org/wiki/Peer_assessment) is a two-way process to help you [pair program](https://en.wikipedia.org/wiki/Pair_programming) your CV. Make sure you're both a giver and receiver of feedback, by playing both *driver* and *navigator* roles in your pair. Playing both roles will help you write a better CV. Giving and receiving feedback will help your start debugging the trickier uknonwn errors: 

1. 🔲 Have you got a second opinion from a “résumé robot”? How robot proof is it? See section \@ref(robotproof) 🤖
1. 🔲 Have you reviewed other people's CV's by doing a CV swap? This will put you in the shoes of an employer or recruiter, thereby helping you to write a better CV yourself, see figure \@ref(fig:cvswap-fig).
...
## CV checklist: triangulation {#triangulation}

You should seek a wide variety of feedback on your CV from as many people as you are comfortable showing it too, this will help you “triangulate” all the different feedback on your CV, a bit like the trigonometry shown in figure \@ref(fig:triangulation-fig).
...
triang_table <- tibble::tribble(
  ...
  "Careers service staff", "Typically lots of experience reading student & graduate CVs",   "May have limited first hand experience of the degree you've studied or the profession you are entering [@samfranklin]", 
  "Recruiters & managers", "Used to reading lots of CVs, hiring people and managing staff",   "Might not know you that well personally, may be biased towards a particular sector or employer", 
  "Peers", "Your fellow students know you, your Uni and course pretty well",   "Probably never had to hire or manage anyone (yet), so limited knowledge of recruitment", 
...
(ref:wartsandall) They know you pretty well, “[warts and all](https://en.wiktionary.org/wiki/warts_and_all)”, see \@ref(fig:honest-fig)

(ref:twentyfourseven) Available online 24-7, see section \@ref(robotproof)

(ref:notcritical) Might not be honest enough or critical enough, see figure \@ref(fig:wonderfuldarling-fig)
...
(ref:worstcritic) It can be difficult to see your own faults, both on paper and in person, see section \@ref(checklist). On the other hand, there is a good chance you might be your own worst critic too. [@worstcritic]
...
(ref:captionhonest) **WANTED (DEAD OR ALIVE) HONEST FRANK FEEDBACK** To improve your CV you want honest frank feedback on it. Frank Feedback and Francesca Feedback aren't judging you, they are judging your CV. Honest feedback will see beyond good or bad and will tell you where you can improve, not just in what you've presented but also what's missing and what you need to focus on in the future to [continue your professional development](https://en.wikipedia.org/wiki/Institute_of_Continuing_Professional_Development) (CPD). So, who are the people you could ask for honest feedback? Can you return their favour and give them some feedback on theirs? [WANTED: Honest Frank Feedback](https://bryanmmathers.com/honest-frank-feedback/) artwork by [Visual Thinkery](https://visualthinkery.com/) is licensed under [CC-BY-ND](https://creativecommons.org/licenses/by-nd/4.0/) 
...
Your future is bright, your future needs debugging. Debugging your future will help you test future. Testing your future will help you to start coding your future. One of the best ways to debug your own CV is to fix bugs in *other peoples* CVs, see figure \@ref(fig:cvswap-fig)
