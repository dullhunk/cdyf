...
This is a self-help guide but a lot of self-help literature can be dry, dull, textbooky, generic and boring with few illustrations and conversations, see section \ref{thinkdifferent} for example. In the novel \emph{\href{https://en.wikipedia.org/wiki/Alice\%27s_Adventures_in_Wonderland}{Alice's Adventures in Wonderland}} \citep{wonderland} shown in figure \ref{fig:aiw-fig}, the protagonist Alice wonders why her sister is reading a book without pictures.
...
\caption{Your Future starts now, not at some moment later in time like when you enter your final year or finish your degree. Artwork by Everydayphrase \href{https://linktr.ee/everydayphrase}{linktr.ee/everydayphrase}}\label{fig:future-now-fig}
\end{figure}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.5769}}
...
You are reading the alpha version, the \href{https://en.wikipedia.org/wiki/Minimum_viable_product}{Minimum Viable Product} (MVP) of this guidebook, last updated on 13 September, 2022. That's software engineer talk for saying \emph{it isn't finished yet}. Subsequent versions, will be continuously and iteratively released on a daily and weekly basis. They will include:
...
\caption{Just like the \href{https://en.wikipedia.org/wiki/Death_Star}{Death Star}, this \sout{galactic superweapon} book is under construction. As of 13 September, 2022 this book is an estimated \textbf{35\% complete}. Image of agile weapon engineering in \emph{Star Wars} via Wikimedia Commons \href{https://w.wiki/5N6q}{w.wiki/5N6q} adapted using the \href{https://apps.apple.com/gb/app/wikipedia/id324715238}{Wikipedia app}}\label{fig:deathstar-fig}
\end{figure}
...
Computing is also an intellectually stimulating and challenging subject to study in its own right. If you don't believe me, I'm not going to make the case here. Have a look at Silvio Peroni's free computational thinking and programming textbook at \href{https://comp-think.github.io/}{comp-think.github.io} which is written for people with a background in the humanities. \citep{peroni} If you like doing \href{https://en.wikipedia.org/wiki/Massive_open_online_course}{Massive Open Online Courses} (MOOCs), you might also enjoy \href{https://en.wikipedia.org/wiki/CS50}{CS50}, with David Malan shown in figure \ref{fig:malan-fig}.
...
\item
  ...
  \tightlist
  \item
    Their \texttt{content} (what you've done) and \texttt{presentation} (how you style your CV)
  \item
...
Lets imagine there is a handy AI-powered function in python called \texttt{find\_jobs()} which knows \emph{everything} about:

\begin{itemize}
\tightlist
\item
  every job available, regardless of where they are advertised, incuding hidden ones
\item
...
Such a function doesn't really exist yet, but lets pretend that it does to illustrate some points about job searching. If you were to run the function with no arguments\ldots{}

\begin{Shaded}
\begin{Highlighting}[]
\ImportTok{from}\NormalTok{ future }\ImportTok{import}\NormalTok{ jobs}

\CommentTok{\# what could possibly go wrong?}
\NormalTok{find\_jobs()}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{\# Fancy managing products?}
\NormalTok{find\_jobs(role}\OperatorTok{=}\NormalTok{product\_management)}

\CommentTok{\# Engineering makes the world go round}
\NormalTok{find\_jobs(role}\OperatorTok{=}\NormalTok{software\_engineering)}

\CommentTok{\# Wannabe a Data Scientist?}
\NormalTok{find\_jobs(role}\OperatorTok{=}\NormalTok{data\_science)}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{\# I\textquotesingle{}ve got the brains}
\NormalTok{find\_jobs(sector}\OperatorTok{=}\NormalTok{BigTech)}

\CommentTok{\# You\textquotesingle{}ve got the looks}
\NormalTok{find\_jobs(sector}\OperatorTok{=}\NormalTok{Entertainment)}

\CommentTok{\# Let\textquotesingle{}s make lots of money}
\NormalTok{find\_jobs(sector}\OperatorTok{=}\NormalTok{FinTech)}
\end{Highlighting}
\end{Shaded}
...
Picking a sector can help you get started, by narrowing what might otherwise be a massive search space. Picking a sector will also determine the kind of salary see section \ref{by-salary}. \citep{letsmakelotsofmoney}

\hypertarget{by-skills}{%
\subsection{By skills}\label{by-skills}}

Is there a specific skill you have you enjoy using, or a technology you know about that has promise? You could use this as a way to find jobs.

\begin{Shaded}
\begin{Highlighting}[]
\ImportTok{from}\NormalTok{ future }\ImportTok{import}\NormalTok{ jobs}

\CommentTok{\# Do you love using Amazon Web Services?}
\NormalTok{find\_jobs(skill}\OperatorTok{=}\NormalTok{AWS)}

\CommentTok{\# Howabout some cutting edge AI?}
\NormalTok{find\_jobs(skill}\OperatorTok{=}\NormalTok{DeepLearning)}

\CommentTok{\# Is your name Noelle Pointer?}
\CommentTok{\# Or are you Neil Pointer?}
\NormalTok{find\_jobs(skill}\OperatorTok{=}\NormalTok{C)}
\CommentTok{\# See https://www.cdyf.me/hacking\#neil{-}pointer}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{\# Hit the North!}
\CommentTok{\# Northern Powerhouse}
\NormalTok{find\_jobs(location}\OperatorTok{=}\NormalTok{Manchester)}

\CommentTok{\# Hit the South!}
\CommentTok{\# Southern Powerhouse}
\NormalTok{find\_jobs(location}\OperatorTok{=}\NormalTok{London)}

\CommentTok{\# Sunshine state}
\NormalTok{find\_jobs(location}\OperatorTok{=}\NormalTok{California)}

\CommentTok{\# location isn\textquotesingle{}t important}
\CommentTok{\# have skills, will travel}
\NormalTok{find\_jobs(location}\OperatorTok{=}\NormalTok{anywhere)}
\end{Highlighting}
\end{Shaded}
...
The people in your network can help you find work. \href{https://en.wiktionary.org/wiki/it\%27s_not_what_you_know_but_who_you_know}{It's not what you know, its who you know}, or so the cliché goes. The bigger your network, the more opportunities you'll know about. So a really basic strategy for finding jobs using your network. Remember, \texttt{find\_jobs} does just know what you know, it also knows who is in your professional \texttt{network} and the employers \emph{they} know about, so we could ask it to find opportunities in your professional and social networks:

\begin{Shaded}
\begin{Highlighting}[]
\ImportTok{from}\NormalTok{ future }\ImportTok{import}\NormalTok{ jobs}

\CommentTok{\# jobs in your network}
\NormalTok{find\_jobs(network}\OperatorTok{=}\VariableTok{True}\NormalTok{)}

\CommentTok{\# jobs outside your network}
\NormalTok{find\_jobs(network}\OperatorTok{=}\VariableTok{False}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{\# are you aiming high?}
\NormalTok{find\_jobs(minimum\_salary}\OperatorTok{=}\NormalTok{£}\DecValTok{36000}\NormalTok{)}

\CommentTok{\# or are you aiming lower?}
\NormalTok{find\_jobs(minimum\_salary}\OperatorTok{=}\NormalTok{£}\DecValTok{18000}\NormalTok{)}

\CommentTok{\# how low can you go?}
\CommentTok{\# is that really a “job”?}
\CommentTok{\# or just shameless exploitation?}
\NormalTok{find\_jobs(minimum\_salary}\OperatorTok{=}\NormalTok{£}\DecValTok{0}\NormalTok{)}

\CommentTok{\# if the salary is so competitive ...  }
\CommentTok{\# why can\textquotesingle{}t they just tell you what it is?}
\NormalTok{find\_jobs(minimum\_salary}\OperatorTok{=}\NormalTok{competitive)}
\end{Highlighting}
\end{Shaded}
...
Do you see yourself working for a large multi-national corporation or a bedroom startup? Or something in-between? There's advantages and disadvantages to each, but like all our other parameters, picking an organisation size (or type) can help you get started. For example:

\begin{Shaded}
\begin{Highlighting}[]
\ImportTok{from}\NormalTok{ future }\ImportTok{import}\NormalTok{ jobs}

\CommentTok{\# looking for large multinationals?}
\NormalTok{find\_jobs(minimum\_employees}\OperatorTok{=}\DecValTok{10000}\NormalTok{)}

\CommentTok{\# howabout something more manageable?}
\NormalTok{find\_jobs(maximum\_employees}\OperatorTok{=}\DecValTok{100}\NormalTok{)}

\CommentTok{\# or even smaller still?}
\NormalTok{find\_jobs(}\BuiltInTok{type}\OperatorTok{=}\NormalTok{startup)}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{\# should we go for a reasonable match?}
\NormalTok{find\_jobs(values\_match}\OperatorTok{=}\DecValTok{70}\OperatorTok{\%}\NormalTok{)}

\CommentTok{\# happy to sell your soul?}
\NormalTok{find\_jobs(values\_match}\OperatorTok{=}\DecValTok{0}\OperatorTok{\%}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

An employers \texttt{values} are often reflected in the organisations \texttt{culture}. What is the employers culture like? Would you enjoy working there and would you felt like you belonged and fitted in? How can you actually find this out? Do you need to sell your soul? If you're struggling with these questions, you may need to do some more soul searching, see section \ref{soul} and chapter \ref{knowing}. Talk to employees too, this is why networking is important, see section @(weakties).

Our \texttt{find\_jobs()} function is useful, if only as a thought experiment, but there's still a strategic question of \emph{when} you use the function and \emph{how often} so lets look at those:
...
\CommentTok{\# jobs that are open now}
\NormalTok{find\_jobs(available}\OperatorTok{=}\VariableTok{True}\NormalTok{)}

\CommentTok{\# jobs that open later in the academic year}
\NormalTok{find\_jobs(available}\OperatorTok{=}\VariableTok{False}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

Armed with knowledge of \emph{when} to apply you can start to target employers. Timing is important because it some cases it will sometimes determine the type of organisation you apply to.
...
\tightlist
\item
  \href{https://www.glassdoor.co.uk/}{glassdoor.co.uk} and \href{https://www.levels.fyi/}{levels.fyi} (``Get Paid, Not Played!'') will give you a good idea for your current and future salary expectations
\item
...
Do you feel like the \emph{weird edge case} pictured in figure \ref{fig:default-user-fig}? Do typical graduate destinations such as large multi-national corporations, not really make you want to \emph{\href{https://en.wikipedia.org/wiki/Shake_Your_Thang}{Shake Your Thang}}? \citep{saltnpepa}
...
Marjorie loves version control, she has very high \href{https://en.wikipedia.org/wiki/Emotional_intelligence}{emotional intelligence} (EQ). Her superpower enables her to recognise emotions in herself and others so that she can resolve people's differences, see figure \ref{fig:margeconflict-fig}.
...
Please note these transcripts are generated with speech to text software and are not perfect word-for-word transcriptions. Some \href{https://en.wikipedia.org/wiki/Speech_disfluency}{speech disfluency} has been manually removed and links, cross references and pictures have been manually added to add extra context. To give clarification, extra (non-quoted) words are given in brackets like this: (some text added afterwards).
