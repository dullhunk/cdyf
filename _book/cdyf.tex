...
You are reading the alpha version, the \href{https://en.wikipedia.org/wiki/Minimum_viable_product}{Minimum Viable Product} (MVP) of this guidebook, last updated on 08 September, 2022. That's software engineer talk for saying \emph{it isn't finished yet}. Subsequent versions, will be continuously and iteratively released on a daily and weekly basis. They will include:
...
\caption{Just like the \href{https://en.wikipedia.org/wiki/Death_Star}{Death Star}, this \sout{galactic superweapon} book is under construction. As of 08 September, 2022 this book is an estimated \textbf{35\% complete}. Image of agile weapon engineering in \emph{Star Wars} via Wikimedia Commons \href{https://w.wiki/5N6q}{w.wiki/5N6q} adapted using the \href{https://apps.apple.com/gb/app/wikipedia/id324715238}{Wikipedia app}}\label{fig:deathstar-fig}
\end{figure}
...
\item
  ...
  \tightlist
  \item
    by role, sector, skills, time, size, quantity, network, salary and location
  \end{itemize}
\item
...
\CommentTok{\# call the fictional function}
\NormalTok{find\_jobs()}
\end{Highlighting}
\end{Shaded}

\ldots you will be completely overwhelmed with the results. It can be bewildering knowing where to start your job search, because the possibilities are endless.

We can break it down, but there are many parameters and variables in your search and the search space can be dauntingly massive. Here are some basic strategies to help you start to \href{https://en.wikipedia.org/wiki/Decomposition_(computer_science)}{decompose} the complex problem of job hunting into smaller, more manageable problems you can start to tackle. We've already mentioned nine parameters:
...
\item
  💰 by salary, see section \ref{money}
\item
  🚛 by size, see section \ref{by-size}
\item
...
Is there a specific skill you have you enjoy using, or a technology you know about that has promise? You could use this as a way to find jobs. Take \href{https://en.wikipedia.org/wiki/Amazon_Web_Services}{Amazon Web Services} (AWS) for example:
...
\ldots{} and so on so locations can help you define your problem space. For example, if you're looking in Manchester, you can get started at \href{https://git.io/manc}{git.io/manc}. Picking a location can also help you identify networking opportunities for hidden, un-advertised vacancies and making speculative applications. You could find out about employers by attending \href{https://www.meetup.com/}{meetup.com} events in your chosen location.

\hypertarget{weakties}{%
\subsection{By network}\label{weakties}}

The people in your network can help you find work. \href{https://en.wiktionary.org/wiki/it\%27s_not_what_you_know_but_who_you_know}{It's not what you know, its who you know}, or so the cliché goes. The bigger your network, the more opportunities you'll know about. So a really basic strategy for finding jobs using your network. Remember, \texttt{find\_jobs} does just know what you know, it also knows who is in your professional \texttt{network} and the employers \emph{they} know about, so we could ask it to find opportunities in our network:
...
\hypertarget{money}{%
\subsection{By salary}\label{money}}
...
or even the dreaded but vague, negotiable and cryptic:
...
\caption{Average graduate starting salaries at the UK's ``top'' employers 2007 to 2022, according to \href{https://www.highfliers.co.uk}{highfliers.co.uk} \citep{highfliers2022} Since 2007, graduate salaries have risen from £24.5k to £32k in 2022. Note that these are \emph{graduate} salaries, not internship or placement salaries which tend to be lower. Also note that ``top'' employers is a subjective judgement, lots of good smaller employers don't feature in this data and may pay less than the large multinational organisations that dominate this set of employers. The averages also hide significant variations by sector and location, see the salary resources in section \ref{undersell}}\label{fig:salaries-fig}
\end{figure}
...
Do you see yourself working for a large multi-national corporation or a bedroom startup? Or something inbetween? There's advantages and disadvantages to each, but like all our other parameters, picking an organisation size can help you get started. For example:
...
will find your jobs in big multinational corporations. Perhaps that's not you:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{find\_jobs(maximum\_employees}\OperatorTok{=}\DecValTok{50}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Our \texttt{find\_jobs()} function is useful, if only as a thought experiment, but there's still the question of \emph{when} you use it and \emph{how often}:
...
\begin{Shaded}
\begin{Highlighting}[]
\SpecialStringTok{* }\NormalTok{“It\textquotesingle{}s October, so I\textquotesingle{}m applying to large employers”}
\end{Highlighting}
\end{Shaded}

Armed with knowledge of \emph{when} to apply you can start to target employers. Timing is important because it some cases it will determine the size of the organisation you apply to.
...
Salaries in the UK for interns (and graduates) range from \href{https://www.gov.uk/national-minimum-wage}{minimum wage} to £50k and over, with everything in between. So beware of under-selling yourself, know your value, see \ref{money}. Some employers see students as a form of cheap labour that can be exploited because you're not ``qualified'' until you graduate. I'd think twice before working for such an employer, computing skills are in demand and there are plenty of other employers who will treat you with more respect. 📉
...
Neil is a mature student who loves the \href{https://en.wikipedia.org/wiki/C_(programming_language)}{C programming language}, see figure \ref{fig:neilpointer-fig}. The Pointer family are sometimes misunderstood, but Neil compensates for this with his excellent memory management skills and efficiency. As well as \href{https://en.wikipedia.org/wiki/The_Pointer_Sisters}{his famous sisters}, \citep{imsoexcited} he has a younger half-brother, Neil Pointer-Exception, from his mothers second marriage. Neil Pointer-Exception prefers \texttt{Java}. Neil Pointer has seen many programming languages come and go, but his favourite will always be \texttt{C}. Born in 1970s, \texttt{C} is easily ``old enough to be your father'' or your mother. Respect your elders! \citep{ritchie}
