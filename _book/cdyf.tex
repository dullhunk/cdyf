...
\item
  Help with existing issues at \href{https://github.com/dullhunk/cdyf/issues}{github.com/dullhunk/cdyf/issues}
\item
...
\textbf{RStudio Tip}: There are two convenient ways to create a new app in RStudio:

\begin{itemize}
\item
  Create a new directory and an \texttt{app.R} file containing a basic app in one step by clicking \textbf{File \textbar{} New Project}, then selecting \textbf{New Directory} and \textbf{Shiny Web Application}.
\item
  If you've already created the \texttt{app.R} file, you can quickly add the app boilerplate by typing ``shinyapp'' and pressing Shift+Tab.
\end{itemize}
...
You are reading the alpha version, the \href{https://en.wikipedia.org/wiki/Minimum_viable_product}{Minimum Viable Product} (MVP) of this guidebook, last updated on 22 May, 2021. That's software engineer talk for saying it isn't finished yet. Subsequent versions, will be continuously and iteratively released on a daily and weekly basis. They will include:
...
Before we get started, let's consider some advice from software engineer \href{https://en.wikipedia.org/wiki/Gayle_Laakmann_McDowell}{Gayle Laakmann McDowell} shown in figure \ref{fig:gayle-fig}. Gayle is an experienced software engineer who has worked at the \href{https://en.wikipedia.org/wiki/Big_Tech}{biggest technology employers in the world}, Apple, Microsoft and Google. She's also authored a cracking series of books on technology careers, particularly \emph{Cracking the Coding Interview} \citep{cracking} which we'll come across in the \href{interviewing.html}{chapter on interviewing}. Gayle refers to the employer ``black hole'':
...
By themselves, those project names are pretty opaque. They are OK for giving the context of your story but don't give the reader much else to go on. Your projects are important because they differentiate you from everyone else in your year group. What was the story (the context, action and result (CAR) we described in section \ref{carstar}) of those projects? How many people were in your team? How long did you collaborate for? What did you build? What was it called? What did it do? What roles and responsibilities did you have in the team? Was their conflict in the team? How did you resolve it? How did you motivate the free-riders in the team to contribute?
...
\SpecialStringTok{* }\NormalTok{Makes use of an HTTP API using secret keys}
...
\SpecialStringTok{* }\NormalTok{Collaborated with one other contributor over two days}
\SpecialStringTok{* }\NormalTok{Tested WidgetWasher on a range of different operating systems}
\SpecialStringTok{* }\NormalTok{Makes use an HTTP API using secret keys}
\SpecialStringTok{* }\NormalTok{WidgetWasher is a web service that washes widgets}
\end{Highlighting}
\end{Shaded}
...
\item
  ✅ Have you quantified and provided evidence of claims you make where you can? See section @ref()
\item
...
\tightlist
\item
  \protect\hyperlink{dogfooding}{Dogfooding} we've already talked about in section \ref{dogfooding}, read your work ALOUD
\item
...
Your actions define your impact, see figure \ref{fig:action-fig}. What are the stories you can tell and the verbs you can use to describe your actions to date? What was the context, action and result (CAR) we discussed in section \ref{carstar}?
...
\caption{Libraries give you power, the power to read your future. This references are your digital library. Picture of the British Museum Reading Room panorama by David Iliff on Wikimedia Commons \href{https://w.wiki/3BEs}{w.wiki/3BEs}}\label{fig:library-power-fig}
\end{figure}
